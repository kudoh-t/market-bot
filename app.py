...
TR, FS = ["reuters.com", "nhk.or.jp", "bloomberg.com", "apnews.com", "nikkei.com"], ["https://feeds.reuters.com/reuters/worldNews", "https://feeds.reuters.com/reuters/businessNews", "https://www.nhk.or.jp/rss/news/cat0.xml"]
# 元コードの全62単語を完全網羅
...
def fy(s):
    try:
        ...
        p = r["regularMarketPrice"]; return p, (p - r["chartPreviousClose"]) / r["chartPreviousClose"] * 100
...
def main():
    ...
    for k, s in ts.items(): p, c = fy(s); d[k+"p"], d[k+"c"] = (p, c) if p is not None else (pre.get(k+"p"), 0.0)
    sp, vn, fn = (d["u10p"] - d["u2p"]) if d["u10p"] and d["u2p"] else 0, [], []
    ...
    sl, fm = min(max(int(sc / 155 * 100), 0), 100), lambda p, c, dc=2: f"{p:.{dc}f}（{c:+.2f}%）" if p else "取得失敗"
    m = [f"【{d['date']} {'🚨戦時モード：総合反転スコア' if vx>=25 or fgi<=20 else '🍀平時モード：トレンドスコア' if vx<=18 and fgi>=40 else '⚠️移行モード'}】\n",
        f"▼ 1. 投資家心理 (FGI)\n 【{next(s for v, s in [(25,'極度の恐怖'),(45,'恐怖'),(55,'中立'),(75,'強欲'),(101,'極度の強欲')] if fgi <= v)}】 指数: {fgi} （前日比：{fgi - d['fgp']:+d}pt）\n",
        ...
        f"▼ 5. 商品 (Commodities)\n ・金 (Gold): {fm(d['gdp'],d['gdc'],1)}\n ・原油(WTI): {fm(d['wtp'],d['wtc'])}\n ・銅 (Cop) : {fm(d['cpp'],d['cpc'],3)}\n 💡 {('🚨【スタ高警戒】' if d['gdc']>0.5 and d['wtc']>1.0 else '📉【景気後退】銅安金高' if d['gdc']>0.5 and d['cpc']<-1.0 else '🏗️【銅の独歩高】'+f'{d['cpc']:+.1f}%' if d['cpc']>1.5 else '⚖️【中立】レンジ内。')}\n",
    ...
    if L_TK: requests.post("https://api.line.me/v2/bot/message/push", headers={"Authorization":f"Bearer {L_TK}","Content-Type":"application/json"}, data=json.dumps({"to":L_ID, "messages":[{"type":"text","text":"\n".join(m)[:4800]}]}))
    else: print("\n".join(m))
