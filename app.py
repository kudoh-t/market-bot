...
KW = ["トランプ", "大統領", "関税", "イラン", "イスラエル", "ホルムズ", "攻撃", "ミサイル", "ウクライナ", "ロシア", "侵攻", "原油", "供給", "opec", "金利", "利上げ", "利下げ", "長期金利", "国債", "インフレ", "デフレ", "物価", "pce", "cpi", "ppi", "frb", "金融政策", "qt", "qe", "gdp", "pmi", "ism", "小売売上高", "住宅着工", "失業率", "景気後退", "景気拡大", "決算", "ガイダンス", "予想上回る", "予想下回る", "eps", "利益率", "売上高", "業績", "中国", "不動産", "恒大", "景気刺激策", "輸出", "減速", "不況", "財政赤字", "政府閉鎖", "インフラ投資", "減税", "規制強化", "テック規制"]
...
def main():
    ...
    for e in [ent for url in FEEDS for ent in feedparser.parse(url).entries]:
        ...
        imp = "重大" if any(k in txt for k in ["ホルムズ","攻撃","ミサイル","封鎖","戦争"]) else "強い影響" if any(k in txt for k in ["雇用統計","cpi","pce","fomc","利上げ","利下げ"]) else "中程度" if any(k in txt for k in ["関税","制裁","緊張","景気後退"]) else "軽微"
        (v_n if any(s in e.link.lower() for s in TRUSTED) else f_n).append({"title": e.title, "impact": imp})

    vix, fgi, spr, score = d["vix_p"], d["fgi_score"], d["spread"], 0
    ...
    msg = [
        f"【{d['date']} {'🚨戦時モード：総合反転スコア' if vix>=25 or fgi<=20 else '🍀平時モード：トレンドスコア' if vix<=18 and fgi>=40 else '⚠️移行モード'}】\n",
        f"▼ 1. 投資家心理 (FGI)\n 【{next(s for v, s in [(25,'極度の恐怖'),(45,'恐怖'),(55,'中立'),(75,'強欲'),(101,'極度の強欲')] if fgi <= v)}】 指数: {fgi} （前日比：{fgi - d['fgi_prev']:+d}pt）\n",
    ...
    with open(CACHE_FILE, "wb") as f_out: pickle.dump(d, f_out)
